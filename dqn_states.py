##########################################
# Solving CartPole with Deep Q-Network （States Input）
# Author: Bruce Zhu
##########################################
...
###################################################################
	def act(self, state, steps_done):
		...
		if a>epsilon:
			pred_ac=torch.max(self.model(Variable(torch.from_numpy(state).float())),0)[1] #return Q value max 
...
###################################################################
	def replay(self, batch_size):
		...
		#print(act_index)
		target_f=self.model(Variable(torch.from_numpy(series).float())).gather(1,act_index)
		#print(target_f)    
		...
		target_a=torch.max(self.model(Variable(torch.from_numpy(next_series).float())),dim=1)[0]
		#print(target_a)
		#print(index)
...
for epoch in range(1, args.epochs+1): #cahnge back
	...
	for steps in range(args.max_step+1): #最多250分
		...
		next_state,reward,done,_=env.step(action) #与环境互动
